...
ForgetAuthWrapper = connect(
    mapAuthStateToProps,
    mapAuthDispatchToProps
)(toJS(ForgetAuthWrapper));
...
class ForgetResetWrapper extends React.Component {
    ...
    render() {
        ...
        if (!locationState || !locationState.forgetInfo || !locationState.forgetInfo.token) {
            return (
                <div className="token-not-found">
                    ...
                    <p>
                        如果没有，请前往
                        <Link to="/passport/forget/auth">验证界面</Link>
                        进行验证
                    </p>
                </div>
            );
...
ForgetResetWrapper = connect(
    mapResetStateToProps,
    mapResetDispatchToProps
)(toJS(ForgetResetWrapper));
