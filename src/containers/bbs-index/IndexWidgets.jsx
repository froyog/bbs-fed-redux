...
AnnounceWrapper = connect(
    mapStateToProps,
    mapDispatchToProps
)(toJS(AnnounceWrapper));

const AppQRCode = () => {
    return (
        <Card className="card-small card-home-widget" title="">
            <img className="mobile-qrcode pull-left" src={mobileQRCode} alt="Mobile QRcode" />
...
class CarouselAdWrapper extends React.Component {
    ...
    render() {
        ...
        return <CarouselAd adList={adList} />;
    }
}
...
class IndexRankWrapper extends React.Component {
    ...
    render() {
        ...
        return (
            <Card className="card-small card-home-widget card-index-rank" title="本周积分排名">
                {isFetching ? <LoadingDots /> : <IndexRank rankData={rankData} />}
            </Card>
        );
    }
}
...
const Footer = () => (
    <footer className="footer-home">
        ...
        <span>津教备0316号</span>
        <br />
        <a href="https://coder.twtstudio.com" target="_blank">
            加入我们
        </a>
