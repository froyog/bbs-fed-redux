...
import { Col, ListGroup, ListGroupItem, Media, Button } from 'react-bootstrap';
    ...
    render() {
        ...
        if (isExpand) {
            ...
            const renderModerators = moderator.length
                ? moderator.map(admin => (
                      <Link className="admin-name" key={admin.uid} to={`/user/${admin.uid}`}>
                          {admin.name}
                      </Link>
                  ))
                : '暂无';
                ...
                <div>
                    <Media className="forum-detail">
                        <Media.Left>
                            <img
                                width={200}
                                height={200}
                                src={`https://bbs.tju.edu.cn/api/forum/${id}/cover`}
                                alt="Forum Cover"
                            />
                        </Media.Left>
                        <Media.Body>
                    ...
                    <ListGroup>
                        {detailedInfo.boards.map(board => {
                            ...
                            return (
                                <LinkContainer to={`/forum/board/${id}/page/1`} key={id}>
                                    <ListGroupItem header={name}>{info}</ListGroupItem>
                                </LinkContainer>
                            );
                        })}
                    </ListGroup>
